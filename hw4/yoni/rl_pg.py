...
class PolicyNet(nn.Module):
    def __init__(self, in_features: int, out_actions: int, **kw):
        ...
        modules.append(nn.Linear(in_features * 2, in_features * 2))
        modules.append(nn.ReLU())
        modules.append(nn.Linear(in_features * 2, out_actions))
        '''
        modules.append(nn.Linear(in_features, 32))
        modules.append(nn.ReLU())
        modules.append(nn.Linear(32, 64))
        modules.append(nn.ReLU())
        modules.append(nn.Linear(64, out_actions))
...
class PolicyAgent(object):
    ...
    def current_action_distribution(self) -> torch.Tensor:
        ...
        # ====== YOUR CODE: ======
        with torch.no_grad():
          actions_proba = nn.functional.softmax(self.p_net(self.curr_state), dim = 0)
        # ========================
    ...
    def step(self) -> Experience:
        ...
        prob_dist = torch.distributions.Categorical(actions_proba) 
        action_taken = prob_dist.sample().item()
        
        self.curr_state, reward, is_done, info = self.env.step(action_taken)
        self.curr_state = torch.tensor(self.curr_state)
        self.curr_episode_reward += reward

        result_tuple = (self.curr_state, action_taken, reward, is_done)
        experience = Experience(*result_tuple)
        # ========================
        if is_done:
            self.reset()
        return experience
...
class VanillaPolicyGradientLoss(nn.Module):
    ...
    def _policy_loss(self, batch, action_scores, policy_weight):
        ...
        action_probs = log_prob.gather(1, batch.actions.view(-1,1)).flatten()
        loss_p = -torch.mean(action_probs * policy_weight) 
        # ========================
        return loss_p
...
class ActionEntropyLoss(nn.Module):
    ...
    def calc_max_entropy(n_actions):
        ...
        # ====== YOUR CODE: ======
        max_entropy = -n_actions*(float(1)/ n_actions * np.log(float(1) / n_actions))
        # ========================
        return max_entropy
    ...
    def forward(self, batch: TrainBatch, action_scores, **kw):
        ...
        # ====== YOUR CODE: ======
        entropy_loss_per_exp = torch.sum(torch.softmax(action_scores, dim=1) * torch.log_softmax(action_scores, dim=1), dim = 1) / self.max_entropy
        loss_e = torch.mean(entropy_loss_per_exp.flatten())
        # ========================
...
class PolicyTrainer(object):
    ...
    def train_batch(self, batch: TrainBatch):
        ...
        for loss_fn in self.loss_functions:
          loss = loss_fn(batch, action_scores)        
          losses_dict.update(loss[1])
          total_loss += loss[0]
