...
def run_experiment(run_name, out_dir='./results', seed=None, device=None,
    ...
    x0,_ = ds_train[0]
    in_size = x0.shape
    num_classes = 10  #because the CIFAR10 class
    channels = []
    for f in filters_per_layer:
        channels.extend([f]*layers_per_block)
    model = model_cls(in_size,num_classes,channels,pool_every,hidden_dims)
    #init training
    loss_fn = torch.nn.CrossEntropyLoss()
    optimizer = torch.optim.SGD(model.parameters(), lr=lr, weight_decay=reg)
    trainer = training.TorchTrainer(model, loss_fn, optimizer, device)
    fit_res = trainer.fit(dl_train,dl_test,epochs,checkpoints,early_stopping,max_batches=batches,print_every=print_every_epoch,**kw)
    # ========================
