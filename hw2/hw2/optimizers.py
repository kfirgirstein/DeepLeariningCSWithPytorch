...
class MomentumSGD(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, momentum=0.9):
        ...
        # ====== YOUR CODE: ======
        self.previous_v = [torch.zeros_like(p[0]) for p in self.params]
        # ========================

    def step(self):
        for (p, dp),v in zip(self.params,self.previous_v):
            if dp is None:
                continue
...
class RMSProp(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, decay=0.99, eps=1e-8):
        ...
        # TODO: Add your own initializations as needed.
        # ====== YOUR CODE: ======
        self.rt = [torch.zeros_like(p[0]) for p in self.params]
        # ========================

    def step(self):
        for (p, dp),r in zip(self.params,self.rt):
            if dp is None:
                continue
            ...
            # parameters tensor.
            # ====== YOUR CODE: ======
            dp_reg= dp+self.reg*p
            r = self.decay*r + (1 - self.decay)*(dp_reg**2)
            coff = self.learn_rate/((r + self.eps)**0.5)
            p -= coff*dp_reg
            # ========================
