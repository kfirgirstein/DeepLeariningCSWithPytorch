...
class KNNClassifier(object):
    ...
    def predict(self, x_test: Tensor):
        ...
        for i in range(n_test):
            ...
            # ====== YOUR CODE: ======
            knn = dist_matrix[:,i].topk(self.k, largest=False)
            knn_lable = self.y_train[knn.indices]
            y_pred[i] = torch.argmax(torch.bincount(knn_lable))
            # ========================
        return y_pred
...
def l2_dist(x1: Tensor, x2: Tensor):
    ...
    if(x1.size(1) == x2.size(1)):
        norm_x1 = (x1**2).sum(1).view(-1, 1)
        norm_x2 = (x2**2).sum(1).view(1, -1)
        dist = norm_x1 + norm_x2 - 2.0 * torch.mm(x1, torch.transpose(x2, 0, 1))
...
def accuracy(y: Tensor, y_pred: Tensor):
    ...
    accuracy = None
    # ====== YOUR CODE: ======
    accuracy = ((y == y_pred).sum()).item() / y.numel()
    # ========================
...
def find_best_k(ds_train: Dataset, k_choices, num_folds):
    ...
    for i, k in enumerate(k_choices):
        ...
        for j in range(num_folds):
            train_subset = torch.utils.data.Subset(ds_train,np.concatenate(indices[0:j] + indices[j+1:]))
