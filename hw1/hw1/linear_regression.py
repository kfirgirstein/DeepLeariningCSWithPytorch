...
class LinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        bias_mask[0][0]=0
        closed_inv = np.linalg.inv(X.T.dot(X)+N*bias_mask*self.reg_lambda)
        w_opt = closed_inv.dot(X.T.dot(y))
        # ========================
...
def cv_best_hyperparams(model: BaseEstimator, X, y, k_folds,
    ...
    params_grid = sklearn.model_selection.ParameterGrid({'bostonfeaturestransformer__degree' : degree_range,'linearregressor__reg_lambda' : lambda_range})
    best_loss = 0
